...
public class RequestRouter {
    ...
    private OpenAIChatRequest translateToModelFormat(OpenAIChatRequest originalRequest, LLMModel model) {
        ...
        if (modelMaxTokens > 0) {
            if (translated.getMaxTokens() == null || translated.getMaxTokens() > modelMaxTokens) {
    ...
    private LLMModel selectFallbackModel(LLMModel failedModel) {
        List<LLMModel> availableModels = modelPool.getActiveModels();
        availableModels.remove(failedModel);

        List<LLMModel> bestModels = modelSelector.selectBestModels(availableModels);
        return loadBalancer.selectModel(bestModels);
    }
